...
// array with constant value so compiler stores the array in flash
// align data so data begins at a flash page boundary
// increment size so data ends at a flash page boundary

#if defined(ARDUINO_ARCH_MM32)
  #define Flash(name, size) \
  __attribute__((__aligned__(1024))) \
  static const uint8_t PPCAT(_data,name)[(size+1023)/1024*1024] = { }; \
  FlashClass name(PPCAT(_data,name), size);

  #define FlashStorage(name, T) \
  __attribute__((__aligned__(1024))) \
  static const uint8_t PPCAT(_data,name)[(sizeof(T)+1023)/1024*1024] = { }; \
...
private:
  const uint32_t PAGE_SIZE_WORDS;
...
public:
  ...
  inline void write(T data)  { flash.write(&data); }
